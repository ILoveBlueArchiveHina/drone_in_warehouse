...
def generate_launch_description():
    kuo_pkg_path = get_package_share_directory('drone_in_warehouse')  # 自己的ros2 套件包位置(share 路徑)
    mavros_pkg_path = get_package_share_directory('mavros')

    pluginlists_yaml = os.path.join(kuo_pkg_path, 'config', 'mavros_pluginlists.yaml')
    config_yaml = os.path.join(kuo_pkg_path, 'config', 'apm_config.yaml')
